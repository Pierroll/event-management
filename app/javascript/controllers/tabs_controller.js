...
export default class extends Controller {
  ...
  showTab(tabId) {
    ...
    const activeTab = (this.element.dataset.tabsActiveTabClass || "").split(" ").filter(Boolean)
    const inactiveTab = (this.element.dataset.tabsInactiveTabClass || "").split(" ").filter(Boolean)
    const activePanel = (this.element.dataset.tabsActivePanelClass || "").split(" ").filter(Boolean)
    const inactivePanel = (this.element.dataset.tabsInactivePanelClass || "").split(" ").filter(Boolean)
